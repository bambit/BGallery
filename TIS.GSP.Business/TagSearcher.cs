...
namespace GalleryServerPro.Business
{
	...
	public class TagSearcher
	{
		...
		/// <summary>
		/// Gets the tags associated with albums the user has permission to view.
		/// </summary>
		...
		private IEnumerable<TagDto> GetTagsForAlbums(IRepository<MetadataTagDto> repo)
		{
			var qry = repo.Where(
				m =>
				m.FKGalleryId == SearchOptions.GalleryId &&
				m.FKTagName.Contains(SearchOptions.SearchTerm) &&
			...
			else
			{
				...
				qry = qry.Where(a => !a.Metadata.Album.IsPrivate);
			}

			return qry.Select(a => a.Tag).Distinct();
		}
		...
		/// modifies it, be sure to check that function to see if it needs a similar change.</remarks>
		private IEnumerable<TagDto> GetTagsForMediaObjects(IRepository<MetadataTagDto> repo)
		{
			var qry = repo.Where(
				m =>
				m.FKGalleryId == SearchOptions.GalleryId &&
